...
| boot_entry_agent | 019ec02f-aedd-78d0-b7ac-efb3f69b6791 | active | 3 | Windows/HotPE wimboot 与 0xc000000f 启动链路诊断 | APPROVED；`0xc000000f` 更像 BCD ramdisk/boot manager 链路问题，不像缺目标硬盘；下一轮菜单应分开测试 wimboot auto 与 explicit `bootmgfw.efi`，避免同项多 boot manager 名称 | 改 menu.ipxe、boot assets、loader、PXE/HTTP Boot 链路时复用 |
