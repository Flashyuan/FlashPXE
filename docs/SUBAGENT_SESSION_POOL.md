...
| webui_agent | Web UI 和管理员体验 | 页面、交互、状态展示；应用 design-review、design-taste-frontend、frontend-design、shadcn-ui、tailwind-design-system 作为 UI 重构检查框架 | active | 019ec02f-b56a-7f83-aa03-0c45c88e8e4b | PXE 仅展示规划/待审核/只读状态；不得擅自引入新前端框架 |
...
| project_decision_agent | 019ec02f-ac76-7863-899a-3af5498ce444 | active | 5 | Web UI/PXE/HotPE 方向决策 | APPROVED，批准在现有轻量静态架构内做免费版体验重构；HotPE/Win11 安装检查属于免费版基础装机能力；暂不批准引入 React/Tailwind/shadcn | 仅在阶段、收费边界、发布方向、商业边界或重大取舍时复用 |
| network_safety_agent | 019ec02f-ad02-7ae0-8c00-c7baeb46709e | active | 6 | Web UI/HotPE readiness first pass 网络安全收口 | APPROVED；未启 DHCP/ProxyDHCP/TFTP、未开放 UDP 67/69/4011、未触碰 TP-Link/OpenWrt/路由/网关/防火墙/DNS；HotPE/Win11 状态展示不是生产 LAN 授权 | 触及 Compose 网络、端口、DHCP、ProxyDHCP、TFTP、路由或网关时必须复用 |
| architecture_agent | 019ec02f-adf7-7162-8721-3b127ac3e51f | active | 3 | Web UI/PXE/HotPE 架构审查 | APPROVED with gates；允许 UI 和只读聚合 API 重构；当前 HotPE 可用性 BLOCKED until 组件提取和真实客户端验证；React/Tailwind/shadcn 第一轮先 BLOCK | 改 API、数据模型、阶段边界或受控 PXE 集成结构时复用 |
...
| webui_agent | 019ec02f-b56a-7f83-aa03-0c45c88e8e4b | active | 3 | Web UI/PXE/HotPE 体验重构建议 | APPROVED first round，保留轻量静态 Web UI，优先重构 Dashboard、镜像页、菜单页、HotPE 页；使用 design-review、design-taste-frontend、frontend-design、shadcn-ui、tailwind-design-system 作为检查框架 | 改管理员 UI、镜像管理体验、状态展示或 API-backed 页面时复用；不得新建同职责 agent；未经架构/决策审查不得引入 React/Tailwind/shadcn/Radix |
...
| security_audit_agent | 019ec02f-baba-7832-bc4a-ed9d8d696048 | active | 6 | Web UI/HotPE readiness first pass 安全审计 | APPROVED；无 secret/token 泄漏、无 raw command 注入、无写 API 越权、无路径越界、无商业实现端点、无误导性 HotPE 已可用或 Win11 已安装验证表述 | 改路径、权限、脚本执行、Docker、安全边界或商业边界时复用 |
| git_audit_agent | 019ec02f-bd09-7520-88db-b57c0d35c296 | active | 5 | Web UI/HotPE readiness first pass Git/发布范围审计 | APPROVED for free-edition stage/commit preparation；未发现商业代码、license、混淆产物或真实 ISO/loader/SQLite/.env/secret/runtime ignored data 进入免费版提交范围 | milestone 收口、stage/commit/push 前必须复用 |
