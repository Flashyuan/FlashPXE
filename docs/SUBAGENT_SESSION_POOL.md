...
| --- | --- | --- | --- | --- | --- | --- |
| research_agent | 019ec02f-abcd-70b2-9364-cdb120d3d2a4 | active | 1 | 固定会话池重建 | READY，负责外部事实、官方资料、协议、TL-ER6120T、iPXE/UEFI/PXE/HTTP Boot 研究；不授权修改主 DHCP 或默认网关 | 仅在 TL-ER6120T、iPXE/UEFI、协议或外部资料不确定时复用 |
| project_decision_agent | 019ec02f-ac76-7863-899a-3af5498ce444 | active | 1 | 固定会话池重建 | READY，负责方向、阶段、免费/商业版边界、发布方向和重大取舍；商业代码不得进入 GitHub 免费版 push | 仅在阶段、收费边界、发布方向、商业边界或重大取舍时复用 |
| network_safety_agent | 019ec02f-ad02-7ae0-8c00-c7baeb46709e | active | 2 | Phase 3.14 实现后网络安全收口 | APPROVED，无阻断项；只读手工声明门禁未启 DHCP/ProxyDHCP/TFTP、未开放 UDP、未触碰 TP-Link/OpenWrt/路由/网关/防火墙/DNS | 触及 Compose 网络、端口、DHCP、ProxyDHCP、TFTP、路由或网关时必须复用 |
| architecture_agent | 019ec02f-adf7-7162-8721-3b127ac3e51f | active | 1 | 固定会话池重建 | READY，负责架构分层、API、数据模型、阶段边界和 PXE/HTTP Boot/ProxyDHCP/TFTP 受控集成结构 | 改 API、数据模型、阶段边界或受控 PXE 集成结构时复用 |
| boot_entry_agent | 019ec02f-aedd-78d0-b7ac-efb3f69b6791 | active | 1 | 固定会话池重建 | READY，负责 iPXE、menu.ipxe、UEFI HTTP/PXE IPv4、loader 和 chainload；区分 readiness 与 boot_tested | 改 menu.ipxe、boot assets、loader、PXE/HTTP Boot 链路时复用 |
...
| security_audit_agent | 019ec02f-baba-7832-bc4a-ed9d8d696048 | active | 2 | Phase 3.14 实现后安全收口 | APPROVED，无阻断项；gate 只读、template-only，不收集/保存真实值，不新增写 API、raw command、配置片段、secret 或生产 LAN 测试入口 | 改路径、权限、脚本执行、Docker、安全边界或商业边界时复用 |
| git_audit_agent | 019ec02f-bd09-7520-88db-b57c0d35c296 | active | 2 | Phase 3.14 Git/发布范围审计 | APPROVED for stage/commit preparation；免费版发布范围可整理提交；未发现商业源码、license、混淆产物进入免费版 push 范围；当前不应直接 push | milestone 收口、stage/commit/push 前必须复用 |
...
| Phase 3.14 isolated lab manual declaration gate | network_safety_agent、security_audit_agent、git_audit_agent | 已实现 `isolated_lab_manual_declaration_gate` API/UI/预检只读手工声明门禁；它只表达进入真实 isolated lab runtime 前必须人工确认的事实模板；`status=missing_facts`、`submission_status=not_submitted`、`authorization_status=not_authorized`、`read_only=true`、`template_only=true`，不收集、不保存真实客户端或实验环境值，不新增写 API，不解锁 runtime | network_safety/security 收口 APPROVED；git_audit_agent APPROVED for stage/commit preparation；`check-phase3-gates.py` PASS；`check-network-safety.sh` PASS；`check-subagent-governance.sh` PASS；`check-compose-config-safe.sh` PASS；`collect-release-evidence.sh` PASS；`node --check apps/web/assets/app.js` PASS；HTTP `/`、`/boot/menu.ipxe`、两个 loader 与 `/api/boot-entry` smoke PASS；API `facts=9`、`missing=9`、runtime/production LAN/boot tested false；UDP 67/69/4011 无监听；`python_bytecode_cache=absent` | 真实 UEFI PXE IPv4 自动启动仍未完成；manual declaration gate 不是用户提交、实验授权、配置生成器、服务启动入口或生产 LAN 授权；push 前需完成本地免费版提交并运行 push readiness |
