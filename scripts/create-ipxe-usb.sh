...
cat <<EOF
SynaBoot Phase 1 iPXE 启动介质说明

本脚本不会自动格式化、分区或写入 U 盘。

请使用你已验证来源的 iPXE USB/ISO/EFI 启动介质，并在 iPXE shell 中执行：

  chain http://${SERVER_IP}:${SYNABOOT_PORT}/boot/menu.ipxe
...
放入真实 loader 后，它们会通过以下 URL 提供：
  ...
  http://${SERVER_IP}:${SYNABOOT_PORT}/boot/loaders/ipxe.efi
  http://${SERVER_IP}:${SYNABOOT_PORT}/boot/loaders/ipxe.iso
EOF
